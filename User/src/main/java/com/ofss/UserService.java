...
public class UserService {
	...
	@Autowired
	private RestTemplate restTemplate; // For interacting with Stock service
	...
	public void buyStock(Long customerId, Long stockId, int quantity) {
		...
		txn.setTransactionId(UUID.randomUUID().getMostSignificantBits() & Long.MAX_VALUE); // generate unique id
	...
	public void sellStock(Long customerId, Long stockId, int quantity) {
		...
		Transaction txn = new Transaction();
		txn.setTransactionId(UUID.randomUUID().getMostSignificantBits() & Long.MAX_VALUE);
		txn.setCustomerId(customerId);
		txn.setStockId(stockId);
